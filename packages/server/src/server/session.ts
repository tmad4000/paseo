...
export class Session {
  ...
  /** Fork-added client-directed messages: UI tab commands and the agent queue. */
  private dispatchForkMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    return this.dispatchUiCommandMessage(msg) ?? this.dispatchAgentQueueMessage(msg);
  }
  ...
  private async dispatchMiscMessage(msg: SessionInboundMessage): Promise<void> {
    // Fork-added client-directed messages (UI tab commands, agent queue) are
    // tried here, in the terminal dispatcher, so the main dispatch chain stays
    // within its complexity budget. Their message types are handled by no other
    // dispatcher, so trying them last is equivalent to an earlier branch.
    const forkHandled = this.dispatchForkMessage(msg);
    if (forkHandled) {
      await forkHandled;
      return;
    }
  ...
  /**
   * Every connected client mirrors the agent message queue, so this is set up
   * once for the session's lifetime rather than gated on observation demand.
   * `agent.queue.update` is a session event, so `emit` fans it out to the
   * clients that subscribed to it via the owned-subscription system.
   */
