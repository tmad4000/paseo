...
describe("ReplicaCache", () => {
  ...
  it("round-trips agent artifacts through the directory cache", async () => {
    const storage = new MemoryStorage();
    const writer = createCache(storage);
    const artifacts = [
      {
    ...
    const dir = directory();
    const cachedAgent = dir.agents.get("agent-1");
    if (!cachedAgent) throw new Error("Expected seeded agent");
    dir.agents.set("agent-1", { ...cachedAgent, artifacts });
    commitDirectory(writer, SERVER_ID, dir);
    await writer.flush();
    ...
    expect(Array.from(restored.agents.keys())).toEqual(["agent-1"]);
    expect(restored.agents.get("agent-1")?.artifacts).toEqual(artifacts);
  });
